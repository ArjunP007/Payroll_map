...
from app.mapper import GPTAdjudicator, map_all as resolve_all, map_batch as resolve_batch
from app.mapper import map_one as resolve_one
from app.schemas import CategoryMappingResponse
from app.validator import (
    validate_category_response_contract,
...
class PayrollMappingEngine:
    ...
    def snapshot(self) -> EngineSnapshot:
        ...
        return EngineSnapshot(
            ...
            prior_code_count=index.total_prior_codes if index is not None else 0,
    ...
    def configure_gpt_integration(self) -> None:
        ...
        if not settings.gpt_missing_prior_fallback_enabled:
            self._gpt_client = None
            logger.info(
                "GPT missing-prior fallback is disabled",
        ...
        logger.info(
            "GPT missing-prior fallback configured: enabled=%s",
    ...
    def prior_codes_by_category(self) -> dict[str, list[str]]:
        """Return known prior codes in source order by category."""

        index = self.require_index()
        return {
            category.value: list(prior_codes)
